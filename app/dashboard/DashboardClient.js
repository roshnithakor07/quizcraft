...
// Run only client-side to avoid hydration mismatch with locale formatting
function useFormattedDate(iso) {
  const [formatted, setFormatted] = useState("");
  useEffect(() => {
    setFormatted(new Date(iso).toLocaleString("en-GB", {
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-[var(--muted)]">
              Welcome back, <span className="font-medium text-[var(--ink)]">{user.name}</span>
            </p>
          </div>
          <div className="flex items-center gap-2">
            <ThemeToggle />
            {/* Refresh */}
            <button onClick={refresh} disabled={refreshing} title="Refresh quizzes"
            ...
            <Link href="/"
              className="text-xs px-3 py-2 border border-[var(--border)] rounded-xl text-[var(--muted)] hover:border-[var(--amber)] transition-all">
              + New quiz
            </Link>
        ...
        <div className="card">
          ...
          {list.length === 0 ? (
            ...
            <div className="space-y-3">
              {list.map(quiz => (
                <div key={quiz.shareId}
                  className="border border-[var(--border)] rounded-xl p-4 hover:border-[var(--amber)]/40 transition-all">
                  ...
                  <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[var(--border)]">
                    ...
                    <Link href={`/results/${quiz.shareId}`} prefetch={true}
                      className="text-xs px-3 py-1.5 border border-[var(--border)] rounded-lg text-[var(--muted)] hover:border-[var(--amber)] hover:text-[var(--ink)] transition-all">
